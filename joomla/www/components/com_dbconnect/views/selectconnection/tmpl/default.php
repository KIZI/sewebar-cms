<?php 
  ...
  if ($this->connections&&(count($this->connections)>0)){
    ...
    foreach ($this->connections as $connection) {
    	echo '<tr>
              ...
              <td>
                <a href="'.JRoute::_('index.php?option=com_dbconnect&task=newDMTask&connection_id='.$connection->id).'" target="_parent">'.JText::_('NEW_DM_TASK').'</a>';
      echo'   </td>
            </tr>';
    }      
    echo '</table>';
  ...
  echo '<a href="'.JRoute::_('index.php?option=com_dbconnect&task=newDatabase',true).'">'.JText::_('SETUP_NEW_DB_CONNECTION').'</a>';
?>