...
class KbiViewSelector extends JView
{
	function display($tpl = NULL)
	{
		...
		if($arbuilder->enabled) {
			...
			$attrs = array(
				'target' => '_blank',
				'onclick' => "window.open(this.href + '&id_query=' + $$('#query').getValue() + '&id_source=' + $$('#sources').getValue() ,'ardesigner','width=1050,height=600,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,copyhistory=no,resizable=yes');return false;",
			);
