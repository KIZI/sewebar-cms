...
class dbconnectController extends JController{
  ...
  private function newEditDMTask(){///TODO - dořešit mapování
    ...
    if ((JRequest::getString('save','')=="ok")&&(@$_POST['name']!='')&&(@$_POST['connection_id']!='')){  
      ...
      if (($_POST['quickDMTask']=='ok')&&($taskId>0)){
        $this->_redirect=JRoute::_('index.php?option=com_dbconnect&task=quickDMTask_generate&task_id='.$taskId.'&generatePreprocessing='.JRequest::getVar('generatePreprocessing'),false);
      }else{
  ...
  public function deleteDMTask(){
    ...
    if (JRequest::getString('xx','xx')==JText::_('DELETE')){
      ...
      $this->_redirect=JRoute::_('index.php?option=com_dbconnect&task=listDMTasks'.$layoutStr,false);
    }elseif (JRequest::getString('xx','xx')==JText::_('STORNO')){
      $this->_redirect=JRoute::_('index.php?option=com_dbconnect&task=listDMTasks'.$layoutStr,false);
    }else{
   ...
   */      
  public function showPreprocessingHints(){
   ...
   */     
  public function selectPreprocessingHint(){
    $maName=JRequest::getString('maName');
    $formatName=JRequest::getString('formatName');
    $pmmlName=JRequest::getString('pmmlName');
    ...
    $this->_redirect=JRoute::_('index.php?option=com_dbconnect&task=preprocessing&task_id='.$taskId,false);
  }
  ...
  public function gotoARDesigner(){
    ...
    $this->_redirect=(str_replace('{$1}', $kbiSource, JText::_('IZI_MINER_URL')));
  ...
  public function generatePMML(){      
    ...
    $view->assign('redirectUrl',str_replace('{$1}', $kbiSource, JText::_('IZI_MINER_URL')));
    $view->display();
  } 
