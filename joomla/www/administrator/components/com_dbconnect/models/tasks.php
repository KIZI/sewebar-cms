...
class dbconnectModelTasks extends JModel
{
	 ...
	 */   	
  public function insertBasicTask($name,$db_table,$columns){
    $db=$this->getDBO();
    $user =& JFactory::getUser();    
    $db->setQuery('INSERT INTO #__dbconnect_tasks (`uid`,`name`,`db_table`,`columns`)VALUES("'.$user->get('id').'","'.$db->getEscaped($name).'","'.$db->getEscaped($db_table).'","'.$db->getEscaped($columns).'")');
    if(!$db->query()){
      return false;
    }else{
      return $db->insertid();
   ...
   */     
  public function updateBasicTask($taskId,$name,$db_table,$columns){
    $db=$this->getDBO();
    $user=&JFactory::getUser();
    $db->setQuery('UPDATE #__dbconnect_tasks SET name="'.$db->getEscaped($name).'",db_table="'.$db->getEscaped($db_table).'",columns="'.$db->getEscaped($columns).'" WHERE id="'.$taskId.'" AND uid="'.$user->get('id').'" LIMIT 1;');
    return $db->query();
  }
