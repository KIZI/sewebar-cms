...
class DBA{
  ...
  public function toJSON(){
    ...
    if (count($this->refs) > 1 && !$this->isAntOrCons) {
      $elements[] = Utils::getBoolean("(", "lbrac");
    }
    
    foreach ($this->refs as $k => $ref) {
...
class AsociationRule{
  ...
  function  __construct($asociationRuleNode, $domER) {
    ...
    foreach ($elements as $element){
      ...
      if ($dba->getId() == $this->antecedent || $dba->getId() == $this->consequent) { $dba->isAntOrCons = true; }
  ...
  public function toJSON(){
    ...
    if ($this->elements[$this->consequent] !== null) {
