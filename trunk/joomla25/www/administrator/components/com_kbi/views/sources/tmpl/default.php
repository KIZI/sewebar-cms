<form action="index.php" method="post" name="adminForm">
	...
	<?php for ($i=0, $n=count( $this->rows ); $i < $n; $i++) : $row = &$this->rows[$i]; ?>
		<tr class="row<?php echo $i % 2; ?>">
			...
			<td align="center">
				<?php if(!empty($row) && (($source = KBIntegrator::create(get_object_vars($row))) instanceof ISynchronable)):?>
					<a href="<?php echo JRoute::_("index.php?option={$option}&controller=documents&id[]={$row->id}") ?>"><?php echo JText::_( 'Documents' ); ?></a>
				<?php else:?>
					-
				<?php endif ?>
			</td>
