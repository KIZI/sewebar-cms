...
class IziController extends JController{
  ...
  public function uploadDemoCSV(){     
    ...
    if ($fileId=$uploadsModel->insertFile('demo_'.$counter.'.csv',$path.'demo_'.$counter.'.csv')){ 
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&tmpl=component&task=uploadCSV_step2&file='.$fileId,false));
      return ;
    } 
    //TODO doplnění chybové zprávy o tom, že se nepodařilo nahrát demo soubor  
  }  
  ...
  public function uploadCSV_step2(){   
    ...
    if (!$fileData){
      ...
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&tmpl=component&task=uploadCSV',false));
      return ;
    }
  ...
  public function cloneDMTask(){        
    ...
    if (@$_POST['action']==JText::_('CREATE_COPY')){ 
      ...
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&tmpl=component&task=listDMTasks',false));
...
//      exit(JRoute::_('index.php?option=com_dbconnect&controller=izi&task=quickDMTask_generate&task_id='.$task->id.'&newAttribute='.$pmmlName,false));
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&task=quickDMTask_generate&task_id='.$task->id.'&newAttribute='.$pmmlName,false));
      return;
    }                                   
    //--kontrola, jestli již byl daný sloupec obsažený v úloze...
  ...
  public function showPreprocessingHint(){       
    ...
    if (!$preprocessingHint){
      ...
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&task=newAttribute&tmpl=component&col='.urlencode($pmmlName).'&task_id='.$taskId,false));
      return ;
    }
  ...
  public function newPreprocessingHint_eachValueOneCategory(){
    ...
    if ($preprocessingHint){
      ...
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&task=quickDMTask_generate&task_id='.$taskId,false));
      return ;
    }
    $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&task=newAttribute&tmpl=component&task_id='.$taskId.'&col='.$pmmlName,false));
  }
  ...
  private function newEditDMTask(){///TODO - dořešit mapování
    ...
    if ((JRequest::getString('save','')=="ok")&&(@$_POST['name']!='')&&(@$_POST['connection_id']!='')){  
      ...
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&task=quickDMTask_generate&task_id='.$taskId.'&generatePreprocessing='.JRequest::getString('generatePreprocessing',''),false));
    }else{                    
  ...
  public function quickDMTask_generate(){ 
    ...
    if(@$_GET['newAttribute']!=''){
      ...
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&task=newAttribute&col='.JRequest::getString('newAttribute','').'&kbi='.$task->kbi_source.'&tmpl=component',false));
    }else{
      $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&task=generatePMML&taskId='.$task->id.'&from=izi&tmpl=component',false));
    }    
  }     
  ...
  public function selectPreprocessingHint(){//TODO !!!
    ...
    if ($attributeName!=''){
      if ($bkefModel->preprocessingHintExists($maName,$formatName,$preprocessingName)){   
        ...
        $this->setRedirect(JRoute::_('index.php?option=com_dbconnect&controller=izi&task=quickDMTask_generate&task_id='.$task->id,false));
        return;
      } 
      //nejspíš nějaká chyba - přesměrujeme uživatele zpátky na výběr preprocessing hintu     
      exit('ERROR - preprocessing hint not found');
      //$this->setRedirect(JRoute::_('index.php?option=com_dbconnect&task=showPreprocessingHints&taskId='.$taskId.'&maName='.urlencode($maName).'&formatName='.urlencode($formatName).'&pmmlName='.urlencode($pmmlName) ,false));
    }else{
  ...
  public function newReportArticle(){
    /** @var $dataModel dbconnectModelConfig */
    $configModel=&$this->getModel('Config','dbconnectModel');
    $categoryId=$configModel->loadConfig('new_report_category');
