...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <link rel="stylesheet" href="components/com_bkef/influences/style.css" type="text/css"/>
        <body onload="uvodniNastaveni();">
            <!-- MAIN PAGE -->
	    <p>Níže je uvedene adresa, která odkazuje na aktuální stránku včetně zaškrtnutých zaškrtávátek, určujících co bude zobrazeno. </p>
            <div id="lokace">
                Adresa: 
            </div>
            <p>Pomocí níže se vyskytujících zaškrtávátek můžete ovlivňovat, které řádky a sloupce
            tabulky budou zobrazeny.</p>
            <div id="checkBoxes">
                <xsl:for-each
                    select="//ds:MetaAttribute[@name and @id and not(@role) and @level = 1]">
                    <xsl:variable name="metaAttribute" select="current()/@name"/>
                    <input type="checkbox"
                        value="{$metaAttribute}" onClick="ShowHideAll(this.value);solveLocation();"/><xsl:value-of select="current()/@name"/>
                </xsl:for-each>
                <xsl:for-each
                    select="//ds:MetaAttributes/ds:MetaAttribute[@level=0][not(//ds:ChildMetaAttribute/@id=@id)]">
                    <xsl:variable name="metaAttribute" select="current()/@name"/>
                    <input type="checkbox"
                        value="{$metaAttribute}" onClick="ShowHideAll(this.value);solveLocation();"/><xsl:value-of select="current()/@name"/>
                </xsl:for-each>
            ...
            <div id="content">
                <div id="BKEF_support2">
                    <div id="BKEF_editInfl" style="display:none;"> </div>
                </div>
                <div id="BKEF_support3">
                    <xsl:apply-templates select="ds:BKEF/ds:Influences"/>
                </div>
            </div>
        </body>
        <script type="text/javascript" src="components/com_bkef/influences/influences.js">window.fg = jk;</script>
        <script type="text/javascript">
				cestaKObrazku='components/com_bkef/influences/img/';
  			</script>
    </xsl:template>
    ...
    <xsl:template match="ds:Influences">
        <table class="matrix">
                    ...
                    <xsl:for-each
                        ...
                        <xsl:choose>
                            <xsl:when
                                ...
                                <td class="attr" name="{$metaAttribute2}" style="display:none;">
                                    <xsl:value-of select="current()/@name"/>
                                    <br/>
                                </td>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:variable name="metaAttribute2" select="current()/@name"/>
                                <td class="attr" name="{$metaAttribute2}" style="display:none;">
                                    <xsl:value-of select="current()/@name"/>
                                    <br/>
                                </td>
                    ...
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:variable name="metaAttribute11"
                                select="current()/parent::*/ds:MetaAttribute[@level = 1]/ds:ChildMetaAttribute[current()/@id = @id]/parent::*/@name"/>
                            <tr name="{$metaAttribute11}" style="display:none;">
                                <td class="attr">
                                    <xsl:value-of select="current()/@name"/>
                                ...
                                <xsl:for-each
                                    ...
                                    <xsl:choose>
                                        <xsl:when
                                            test="current()/parent::*/ds:MetaAttribute[@level = 1]/ds:ChildMetaAttribute[current()/@id = @id]/parent::*/@name">
                                            <xsl:variable name="metaAttribute111"
                                                select="current()/parent::*/ds:MetaAttribute[@level = 1]/ds:ChildMetaAttribute[current()/@id = @id]/parent::*/@name"/>


                                            <!-- 2nd VAR // Actual value from 2nd iteration block -->
                                            <xsl:variable name="attributeII"
                                                select="current()/@name"/>
                                    ...
                                    /@type"/>
                                                  ...
                                                  <xsl:with-param name="groupAttribute"
                                                  select="$metaAttribute111"/>
                                                  ...
                                                  <xsl:when
                                                  test="$selectAttribute = $positiveBoolean">
                                                  <xsl:call-template name="ds:Cell">
                                                  <xsl:with-param name="selectAttribute"
                                                  select="$selectAttribute"/>
                                                  <xsl:with-param name="groupAttribute"
                                                  ...
                                                  <xsl:when
                                                  test="$selectAttribute = $negativeBoolean">
                                                  <xsl:call-template name="ds:Cell">
                                                  <xsl:with-param name="selectAttribute"
                                                  select="$selectAttribute"/>
                                                  <xsl:with-param name="groupAttribute"
                        ...
                        <xsl:otherwise>
                            ...
                            <xsl:variable name="metaAttribute1111" select="current()/@name"/>
                            <tr name="{$metaAttribute1111}" style="display:none;">
                                <td class="attr">
                                    <xsl:value-of select="current()/@name"/>
