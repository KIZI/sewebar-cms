        ...
        String query =
                "declare function local:processRequest($request as node()) {"
                    ...
                    + "\n let $attribs := for $MBA in $generalSet/MandatoryPresenceConstraint/MandatoryBA/text() return "
                        + "\n for $DBA in $request//DBASetting[@id = $MBA] return local:DBAtoBBARecursion($DBA//BASettingRef, $request, \"\", \"\", false())"
                    ...
                    + "\n else <IM id=\"{$im/@id}\"><InterestMeasure>{$im/InterestMeasure/text()}</InterestMeasure><Threshold>{$im/Threshold/text()}</Threshold><CompareType>{$im/CompareType/text()}</CompareType></IM>}</IMs></AR_query>"
                    + ") else ("
                    ...
                    + "\n else for $DBA in $request//DBASetting[@id = $ante] return <DBA connective=\"{$DBA/@type}\" match=\"{$DBA/@match}\">{local:DBAtoBBARecursion($DBA//BASettingRef, $request, \"\", \"\", false())}</DBA>"
                    ...
                    + "\n else for $DBA in $request//DBASetting[@id = $cons/text()] return <DBA connective=\"{$DBA/@type}\" match=\"{$DBA/@match}\">{local:DBAtoBBARecursion($DBA//BASettingRef, $request, \"\", \"\", $exception)}</DBA>"
                    ...
                    + "\n else for $DBA in $request//DBASetting[@id = $cond] return <DBA connective=\"{$DBA/@type}\" match=\"{$DBA/@match}\">{local:DBAtoBBARecursion($DBA//BASettingRef, $request, \"\", \"\", false())}</DBA>"
                    + "\nreturn"
                ...
                + "\n declare function local:DBAtoBBARecursion($BARefs as node()*, $request as node(), $literal as xs:string*, $inference as xs:string*, $exception as xs:boolean){"
                    + "\n for $odkaz in $BARefs let $excep := if ($exception) then if ($literal = \"\" or empty($literal)) then \"Negative\" else if($literal=\"Positive\") then \"Negation\" else if ($literal=\"Negation\") then \"Positive\" else() else () let $liter := if ($exception) then $excep else if ($literal = \"\" or empty($literal)) then \"Both\" else $literal let $infer := if ($inference = \"\" or empty($inference)) then \"false\" else $inference return "
                        + "\n if (count($request//BBASetting[@id = $odkaz/text()])>0) then <DBA connective=\"{$liter}\" inference=\"{$infer}\">{local:BBABuild($request//BBASetting[@id = $odkaz/text()], $request//DictionaryMapping)}</DBA> else local:DBAtoBBARecursion($request//DBASetting[@id = $odkaz/text()]//BASettingRef, $request, $request//DBASetting[@id = $odkaz/text()]/LiteralSign/text(), $request//DBASetting[@id = $odkaz/text()]/LiteralSign/@inference, $exception)};"
