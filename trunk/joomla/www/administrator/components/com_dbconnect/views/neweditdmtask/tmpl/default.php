...
defined('_JEXEC') or die('Restricted access');
  ...
  <?php
  echo '<h1>'.(@$this->dmTask?JText::_('EDIT_DM_TASK_H1'):JText::_('NEW_DM_TASK_H1')).'</h1>';
  if (isset($this->dmTask)){
    echo '<h2>'.$this->dmTask->name.'</h2>';
  }       
  if ($this->columns&&(count($this->columns)>0)){
    ...
    echo '<table>
            <tr>
              ...
              <td>
                <input type="text" name="name" value="'.(@$_POST['name']!=""?$_POST['name']:@$this->dmTask->name).'" maxlength="60" class="textInput" />
              </td>
            </tr>
          </table>';
    ...
    foreach ($this->columns as $column) {
      ...
      echo '<tr class="row'.$rowClass.'">';
    	echo '<td><input type="checkbox" name="'.$column['Field'].'_useColDM"  id="'.$column['Field'].'_use" '.($disabled?'disabled="disabled"':'').' value="1" onchange="showItemText(this);" class="columnCheckbox" '.((@$columnData['use']||@$_POST[$column['Field'].'_use'])?'checked="checked"':'').' /></td>';
    ...
    echo '</table>'; 
    echo '<div class="actionsDiv"><input type="submit" value="'.JText::_('SAVE_TASK').'" class="button disabled" disabled="disabled" id="taskSubmitButton" /></div>';
    echo '</form>';
  }
