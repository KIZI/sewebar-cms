...
class BkefViewSelectArticle extends JView
{
  ...
  function articlesHtml(){
    ...
    $result.= '<form action="index.php?option=com_bkef&amp;task=influences" name="adminForm" id="adminForm" method="post">';
    ...
    if (($total>0)&&(count($articles)>0)) foreach ($articles as $article) {
      ...
      $result.= '<tr class="'.$rowClass.'"><td><a href="index.php?option=com_bkef&amp;task=influences&amp;article='.$article->id.'">'.$article->title.'</a></td><td>'.$article->section.'</td><td>'.$article->categorie.'</td><td>'.$article->cdate.'</td></tr>';
    }
  ...
  {   
    ...
    echo JText::_('SELECT_ARTICLES_INFLUENCES_ABOUT');
