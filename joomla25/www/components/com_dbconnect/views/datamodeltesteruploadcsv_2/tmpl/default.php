...
defined('_JEXEC') or die('Restricted access');
  ...
  echo '<div id="iziuploadcsv2Div">
          <form method="post" action="'.JRoute::_('index.php?option=com_dbconnect&controller=data&task=modelTesterUploadCSV_import').'" >
            <input type="hidden" name="file" value="'.$this->fileData->id.'" />
            <input type="hidden" name="lmtask" value="'.$this->lmtask.'" />
            <input type="hidden" name="kbi" value="'.$this->kbi.'" />
            <input type="hidden" name="rules" value="'.$this->rules.'" />
            <input type="hidden" id="table_name" name="table_name" value="testdata" />
            ...
            <input type="submit" value="'.JText::_('TEST_CLASSIFICATION_MODEL').'" class="bigButton" />
          </form>
