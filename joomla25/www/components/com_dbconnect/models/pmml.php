  ...
  class dbconnectModelPmml extends JModel{
       ...
       if (count($columns->column)>0){
         foreach ($columns->column as $column) {   
           ...
           if ($fmlModel->hasPreprocessingForPmmlField($pmmlFieldName)){  
             ...
             //projdeme všechny zadané preprocessing hinty a pokusíme se je vygenerovat
             $preprocessings=$bkefInfoArr['preprocessings'];
             if (count($preprocessings->Preprocessing)>0){
               foreach ($preprocessings->Preprocessing as $preprocessing){
               	 //vygenerování atributu na základě konkrétního preprocessing hintu
                 $bkefPreprocessing=$bkefModel->getPreprocessingHint($bkefInfoArr['metaattributeName'],$bkefInfoArr['formatName'],@$preprocessing->PreprocessingHint);
                 if (!$bkefPreprocessing){continue;}
                 $attributeName=@$preprocessing->AttributeName;
                 if ($attributeName==''){$attributeName=$pmmlFieldName;}
                 //máme zadané jméno atributu a preprocessing, musíme ověřit, jestli je jméno atributu unikátní a poté provést generování
                 $bkefFormat=$bkefModel->getFormat($bkefInfoArr['metaattributeName'],$bkefInfoArr['formatName']);
                 ...
                 $derivedName=$attributeName;
                 $derivedCounter=1;
                 while (in_array($derivedName,$derivedFieldNamesArr)){
                   $derivedCounter++;
                   $derivedName=$attributeName.' ('.$derivedCounter.')';
                 }
                 $derivedFieldNamesArr[]=$derivedName;
    ...
    private function discretize_intervalEnumeration(&$discretize,$intervalEnumeration){
      if (count($intervalEnumeration->IntervalBin)>0){
        foreach ($intervalEnumeration->IntervalBin as $intervalBin) {
          ...
          if (isset($intervalBin->Name)){$intervalName=(string)$intervalBin->Name;}else{$intervalName='intervals';}
          //TODO tady by to chtělo lepší kontrolu jména
    ...
    private function discretize_equidistant(&$discretize,$equidistant){
      ...
      if (($start<$end)&&($step>0)){
        ...
        $stepTeckaPos=strpos($step,'.');
        if ($stepTeckaPos>0){
          $stepDesetinnaCast=substr($step,$stepTeckaPos+1);
          $pocetDesetinnychMist=strlen($stepDesetinnaCast);
        }else{
          $pocetDesetinnychMist=0;
        }
        $stepTeckaPos=strpos($start,'.');
        if ($stepTeckaPos>0){
          $stepDesetinnaCast=substr($start,$stepTeckaPos+1);
          $pocetDesetinnychMist2=strlen($stepDesetinnaCast);
          $pocetDesetinnychMist=max($pocetDesetinnychMist,$pocetDesetinnychMist2);
        }
        //--určení počtu desetinných míst
        ...
        while ($start<$end){ 
          ...
          if ($endX>=$end){
            ...
            $endX=round($endX,$pocetDesetinnychMist);
            $closure.='Open';
          ...
          $start+=$step;
          $start=round($start,$pocetDesetinnychMist);
        }
      }
    }
