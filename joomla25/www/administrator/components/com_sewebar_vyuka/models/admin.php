  ...
  class sewebarModelAdmin extends JModel{
    ...
    public function addUserGroup($parentId,$title){    
      ...
      if (isset($usergroup->rgt)){
        ...
        $db->setQuery("UPDATE #__usergroups SET lft=lft+2 WHERE lft>'".$usergroup->rgt."';");
        $db->query();
    ...
    public function addUser($jmeno,$username,$email,$mainGroupId){
      $db=$this->getDBO();
      $db->setQuery("SELECT * FROM #__users WHERE username='".$db->getEscaped($username)."' OR email='".$db->getEscaped($email)."' LIMIT 1;");
    ...
    public function removeUserFromGroupsChildren($userId,$parentGroupId){
      $db=$this->getDBO();
      $db->setQuery("DELETE FROM #__user_usergroup_map WHERE user_id=".$db->quote($userId)." AND group_id IN (SELECT id FROM #__usergroups WHERE parent_id=".$db->quote($parentGroupId).")");
      $db->query();
    }
