...
defined('_JEXEC') or die('Restricted access');
  ...
  $headerLink='index.php?option=com_dbconnect&task=adminConnections&order=';
  if ($this->connections&&(count($this->connections)>0)){
    ...
    echo '<tr>
            ...
            <th><a href="'.JRoute::_($headerLink.'joomla_name').'">'.JText::_('CREATED_BY_USER').'</a></th>
            <th>Akce</th>
          </tr>';
    foreach ($this->connections as $connection) {
    	echo '<tr>
              ...
              <td>'.$connection->joomla_name.' ('.$connection->joomla_username.')</td>
              <td class="actionsTd">
                <a href="'.JRoute::_('index.php?option=com_dbconnect&task=showTable&tmpl=component&connection_id='.$connection->id).'" class="modal">'.JText::_('SHOW_DB_TABLE_PREVIEW').'</a>
                <a href="'.JRoute::_('index.php?option=com_dbconnect&task=newDMTask&connection_id='.$connection->id).'">'.JText::_('NEW_DM_TASK').'</a>
                <a href="'.JRoute::_('index.php?option=com_dbconnect&task=shareConnection&connection_id='.$connection->id).'">'.($connection->shared?(JText::_('DISABLE_SHARING')):(JText::_('ENABLE_SHARING'))).'</a>
                <a href="'.JRoute::_('index.php?option=com_dbconnect&task=deleteConnection&tmpl=component&connection_id='.$connection->id).'" class="modal">'.JText::_('DELETE').'</a>
              </td>
            </tr>';
    }      
    echo '</table>';
