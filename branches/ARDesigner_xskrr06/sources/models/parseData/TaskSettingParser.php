    ...
    public function __construct($bbaNode) {
        ...
        foreach ($bbaChildren as $bbaChild) {
            ...
            if ($bbaChild->nodeName == "Coefficient") {
                $fieldChildren = $bbaChild->childNodes;
                $name = "";
                foreach ($fieldChildren as $fieldChild) {
                    if ($fieldChild->nodeName == "Type") {
                        $name = $fieldChild->nodeValue;
                        $this->category = $name;
                    }
                    if ($name != "" && $name != "One category") {
                        if ($fieldChild->nodeName == "MinimalLength") {
                            $minLength = $fieldChild->nodeValue;
                            $this->catRef[] = new TaskSettingBBAField("Minimal Length", $minLength);
                        }
                        if ($fieldChild->nodeName == "MaximalLength") {
                            $maxLength = $fieldChild->nodeValue;
                            $this->catRef[] = new TaskSettingBBAField("Maximal Length", $maxLength);
                        }
                    } else {
                        if ($fieldChild->nodeName == "Category") {
                            $value = $fieldChild->nodeValue;
                            $this->catRef[] = new TaskSettingBBAField("One category", $value);
                        }
                    }
                }
            }
...
class TaskSettingRule {
    ...
    function __construct($asociationRuleNode, $domER) {
        ...
        foreach ($elements as $element) {
            $bba = new TaskSettingBBA($element, $this->elements);
            $this->elements[$bba->getId()] = $bba;
        }
        $elements = $domER->getElementsByTagName('DBASetting');
        foreach ($elements as $element) {
            $dba = new TaskSettingDBA($element, $this->elements);
